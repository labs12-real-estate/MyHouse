...
class AddressInput extends Component {
    ...
    handleInputChange = e => {
        ...
        if ([3, 6, 10, 15].includes(string.length)) {
            ...
            const url = `https://cors-anywhere.herokuapp.com/https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${string}&types=${this.types}&location=${this.location}&radius=500&key=${this.key}&sessiontoken=${this.sessiontoken}`;
            axios
                .get(url)
                .then(res => {
                    this.setState(prevState => ({
        ...
        return (
            <div>
                ...
                {this.state.predictions.map(prediction => (
                    ...
                    <button 
                        key={prediction.id}
                        onClick={e => this.fillAddress(prediction.description, e)}
                    >
