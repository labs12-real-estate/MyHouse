export default function calculatePayments(initial, years, rate, monthlyOverpayment, overpayments = []) {
  const monthlyRatePct = rate / 1200;
  const monthlyPayment = monthlyRatePct === 0 ? initial / years / 12 : (initial * monthlyRatePct) / (1 - Math.pow(1 / (1 + monthlyRatePct), years * 12));
  let balance = initial;
  ...
  for (let year = 0; year < years; year++) {
    ...
    for (let month = 1; month <= 12; month++) {
      const overpayment = overpayments.filter(x => +x.year === year && +x.month === month).reduce((acc, val) => acc + +val.amount, 0);
